...
if [ "$1" != "nogui" ] ; then
	if [ "$pruned" == "true" ] ; then
		...
		echo
		src/qt/bitcoin-qt -prune=550 -datadir="$dir" $params
	else
		echo
		src/qt/bitcoin-qt -datadir="$dir" $params
	fi
else
	...
	if [ "$pruned" == "true" ] ; then
		echo "Pruned mode activated, only keeping 550 block transactions"
		echo
		src/bitcoind -prune=550 -datadir="$dir" $params #-reindex-chainstate
	else
		echo
		src/bitcoind -datadir="$dir" -txindex=1 $params
		# Reindexing the chainstate:
		#src/bitcoind -datadir="/media/sf_Bitcoin" -reindex-chainstate
		
		# Reindexing the transaction index database
		#src/bitcoind -datadir="$dir" -txindex=1 -reindex $params
	fi
fi
